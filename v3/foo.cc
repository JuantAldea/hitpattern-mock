...
int foo()
{
    ...
    for (int i=0; i < 10; i++){
        ...
        for (int j = 0; j < 5; j++){
            tracks.back().setHitPattern(j);
        }
        std::cout << "Inserted track " << i << std::endl;
    }
    m_rootTree->Branch("collection-split2", &tracks, 32000, 2);
    m_rootTree->Branch("collection-split1", &tracks, 32000, 1);
    m_rootTree->Branch("collection-split0", &tracks, 32000, 0);
